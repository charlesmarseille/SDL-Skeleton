...
dataset_name = 'nord_cotiere' # ryam, nordCotiere
save_path = f'D:/cmarseille/reseaux/predictions/{dataset_name}/'
...
elif dataset_name == 'nord_cotiere':
...
net.load_state_dict(torch.load(f'./Ada_LSN/weights/inception_nord_cotiere_lr1e-6_c64/skel_{epoch}.pth', map_location=lambda storage, loc: storage))
...
for tile in tiles:
    ...
    seg_file = rasterio.open(
        save_path+model_name+'_'+tile+'epoch'+str(epoch)+'.tif',
        'w+',
        BIGTIFF=True,
        **seg_kwds
    )
